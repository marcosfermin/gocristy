...
class Upgrade extends CI_Controller {
    ...
    public function _init() {
        ...
        $this->cur_version = $this->GoCristy_model->getVersion();
        $this->last_version = $this->GoCristy_admin_model->getLatestVersion();
    }
    ...
    public function index() {
        ...
        $this->session->unset_userdata('gocristy_lastver');
    ...
    public function download() {
        ...
        $lastversion = $this->GoCristy_admin_model->chkVerUpdate($this->cur_version);
        if ($lastversion !== FALSE) {
            $nextversion = $this->GoCristy_admin_model->findNextVersion($this->cur_version);
            $url1 = $this->config->item('GoCristy_upgrade_server_1') . "upgrade-to-" . $nextversion . ".zip";
            $url2 = $this->config->item('GoCristy_upgrade_server_2') . "upgrade-to-" . $nextversion . ".zip";
            ...
            if($this->GoCristy_model->downloadFile($url, $newfname) !== FALSE){
                $this->GoCristy_admin_model->setMaintenance();
                ...
                if($this->GoCristy_admin_model->chkVerUpdate($this->GoCristy_model->getVersion()) !== FALSE){
                    ...
                    $this->GoCristy_admin_model->unsetMaintenance();
                    $this->GoCristy_model->clear_all_cache();
                    $this->db->cache_delete_all();
                    // When Success 
    ...
    public function fileBackup() {
        ...
        foreach (directory_map('templates/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html" && $t != ".htaccess") && $t != "admin" && $t != "gocristydefault") {
                    $this->zip->read_dir('templates/' . $t . '/');
                }
            }
        }
        foreach (directory_map('gocristy/views/templates/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html") && $t != "admin" && $t != "gocristydefault") {
                    $this->zip->read_dir('gocristy/views/templates/' . $t . '/');
                }
            }
        }
        foreach (directory_map('gocristy/config/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "article.php" && $t != "gallery.php") {
                    $this->zip->read_file('gocristy/config/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/language/english/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "article_lang.php" && $t != "gallery_lang.php") {
                    $this->zip->read_file('gocristy/language/english/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/language/dutch/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "article_lang.php" && $t != "gallery_lang.php") {
                    $this->zip->read_file('gocristy/language/dutch/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/language/italian/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "article_lang.php" && $t != "gallery_lang.php") {
                    $this->zip->read_file('gocristy/language/italian/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/language/spanish/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "article_lang.php" && $t != "gallery_lang.php") {
                    $this->zip->read_file('gocristy/language/spanish/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/language/thai/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "article_lang.php" && $t != "gallery_lang.php") {
                    $this->zip->read_file('gocristy/language/thai/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/controllers/admin/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "Article.php" && $t != "Gallery.php") {
                    $this->zip->read_file('gocristy/controllers/admin/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/modules/plugin/controllers/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "Article.php" && $t != "Gallery.php") {
                    $this->zip->read_file('gocristy/modules/plugin/controllers/' . $t, TRUE);
                }
            }
        }
	foreach (directory_map('gocristy/models/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if ($t != "index.html" && $t != "Article_model.php" && $t != "Gallery_model.php") {
                    $this->zip->read_file('gocristy/models/plugin/' . $t, TRUE);
                }
            }
        }
        foreach (directory_map('gocristy/modules/plugin/views/templates/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html") && $t != "gocristydefault") {
                    $this->zip->read_dir('gocristy/modules/plugin/views/templates/' . $t . '/');
                }
            }
        }
        foreach (directory_map('gocristy/modules/plugin/views/templates/gocristydefault/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html") && $t != "article" && $t != "gallery") {
                    $this->zip->read_dir('gocristy/modules/plugin/views/templates/gocristydefault/' . $t . '/');
                }
            }
        }
        foreach (directory_map('gocristy/views/admin/plugin/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html") && $t != "article" && $t != "gallery") {
                    $this->zip->read_dir('gocristy/views/admin/plugin/' . $t . '/');
                }
            }
        }
        foreach (directory_map('gocristy/views/frontpage/templates/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html") && $t != "gocristydefault") {
                    $this->zip->read_dir('gocristy/views/frontpage/templates/' . $t . '/');
                }
            }
        }
        unset($t);
    ...
    public function clearAllSession() {
        ...
        @array_map('unlink', glob(FCPATH . DB_NAME . '_*'));
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        $this->GoCristy_model->clear_all_session();
    ...
    public function clearAllErrLog() {
        ...
        admin_helper::is_allowchk('maintenance');
        admin_helper::is_allowchk('delete');
        $this->GoCristy_model->clear_all_error_log();
        @array_map('unlink', glob(FCPATH . EMAIL_DOMAIN . '_*'));
        @array_map('unlink', glob(FCPATH . DB_NAME . '_*'));
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect('admin/upgrade', 'refresh');
