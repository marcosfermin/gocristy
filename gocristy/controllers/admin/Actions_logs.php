...
class Actions_logs extends CI_Controller {
    ...
    public function index() {
        ...
        $result_per_page = 50;
        $total_row = $this->GoCristy_admin_model->countTable('actions_logs', $search_arr);
        $num_link = 10;
        $base_url = $this->GoCristy_model->base_link(). '/admin/actionslogs/';
        // Pageination config
        $this->GoCristy_admin_model->pageSetting($base_url,$total_row,$result_per_page,$num_link);
        ($this->uri->segment(3))? $pagination = ($this->uri->segment(3)) : $pagination = 0;
        //Get users from database
        $this->template->setSub('actions_logs', $this->GoCristy_admin_model->getIndexData('actions_logs', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function delActionsLogs() {
        ...
        if(isset($delR)){
            foreach ($delR as $value) {
                if ($value) {
                    $this->GoCristy_admin_model->removeData('actions_logs', 'actions_logs_id', $value);
                }
            }
        }
