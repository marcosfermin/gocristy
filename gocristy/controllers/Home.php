...
class Home extends CI_Controller {
    ...
    public function _init() {
        ...
        if(strpos($pageURL, 'plugin') !== FALSE){
            redirect($this->GoCristy_model->base_link().'/'.$pageURL);
        }
        $this->page_url = $pageURL;
        $this->page_rs = $this->GoCristy_model->load_page($pageURL);
        ...
        if ($page_rs !== FALSE) {
            $this->template->set('core_css', $this->GoCristy_model->coreCss($page_rs->custom_css, FALSE));
            $this->template->set('core_js', $this->GoCristy_model->coreJs($page_rs->custom_js, FALSE));
            $title = $page_rs->page_title . ' | ' . $row->site_name;
            $this->template->set('title', $title);
            $this->template->set('meta_tags', $this->GoCristy_model->coreMetatags($page_rs->page_desc, $page_rs->page_keywords, $title, '', $page_rs->more_metatag));
            ...
            $this->template->set('title', $title);
            $this->template->set('meta_tags', $this->GoCristy_model->coreMetatags('404 Page not Found',$row->keywords,$title));
    ...
    public function index() {
        ...
        if($config->maintenance_active){
            ...
            if($this->page_rs === FALSE){
                ...
                $this->template->setSub('content', $this->GoCristy_model->getHtmlContent($this->page_rs->content, $this->uri->segment($totSegments)));
                $this->template->loadSub('frontpage/getpage');
            }
            if ($this->GoCristy_model->findFrmTag($this->output->get_output(), TRUE) !== false) {
        ...
        if($config->maintenance_active){
            ...
            $html = '<h1>Sorry, Page not Found!</h1>
                            <p>Sorry! Page not Found. (' . current_url() . ') <br>Please back to home page.<p>
                                <a class="btn btn-primary btn-lg" href="' . $this->GoCristy_referrer->getReferrer() . '" role="button">back to home &raquo;</a>';
    ...
    public function setLang() {
        $this->GoCristy_model->setSiteLang($this->uri->segment(2));
    ...
    public function getCoreCSS(){
        ...
        if (isset($_SERVER['SERVER_PROTOCOL']) && isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && isset($_SERVER['HTTP_IF_NONE_MATCH']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == $fileModified && trim($_SERVER['HTTP_IF_NONE_MATCH']) == $etag) {
            ...
            echo $this->GoCristy_model->setJSCSScache($cssFiles, 'corecss', 'css', $expires);
        } else {
            $this->GoCristy_model->setJSCSSheader($fileModified, $expires, $etag, 'text/css');
            echo $this->GoCristy_model->setJSCSScache($cssFiles, 'corecss', 'css', $expires);
        }
    ...
    public function getCoreJS(){
        ...
        if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && isset($_SERVER['HTTP_IF_NONE_MATCH']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == $fileModified && trim($_SERVER['HTTP_IF_NONE_MATCH']) == $etag) {
            header($_SERVER['SERVER_PROTOCOL'] . ' 304 Not Modified');
            echo $this->GoCristy_model->setJSCSScache($jsFiles, 'corejs', 'js', $expires);
        } else {
            $this->GoCristy_model->setJSCSSheader($fileModified, $expires, $etag, 'text/javascript');
            echo $this->GoCristy_model->setJSCSScache($jsFiles, 'corejs', 'js', $expires);
        }
        $this->output->cache(43200);
        exit(0);
