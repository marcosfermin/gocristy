 ...
 * 
 * Fixed for support by Marcos Fermin (Founder of GOCRISTY CMS)
 *
 * Example:
...
class RSSParser {
	...
	public $cache_dir 		= './gocristy/cache/'; 	// Cache directory
	...
	{
            $CI =& get_instance();
            $CI->load->model('GoCristy_model');
            if($CI->GoCristy_model->is_url_exist($this->feed_uri) !== FALSE){
		// Are we caching?
		...
		if (!isset($rawFeed))
		{
			$rawFeed = @$CI->GoCristy_model->get_contents_url($this->feed_uri);
                        $rawFeed = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $rawFeed);
                        $rawFeed = preg_replace('#&(?=[a-z_0-9]+=)#', '&amp;', $rawFeed);
                        $rawFeed = preg_replace('#<content:encoded>(.*?)</content:encoded>#is', '', $rawFeed);
		}
	...
	*/
	...
	function runFeedBackend($num)
	{
            ...
            if (!$CI->cache->get('backend_rssfeed_news')) {
                ...
                ($CI->GoCristy_model->load_config()->pagecache_time == 0) ? $cache_time = 1 : $cache_time = $CI->GoCristy_model->load_config()->pagecache_time;
