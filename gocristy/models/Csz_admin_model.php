...
class GoCristy_admin_model extends CI_Model{

    function __construct(){
        parent::__construct();
        $this->load->model('GoCristy_model');
        if (CACHE_TYPE == 'file') {
    ...
    public function load_config(){
        return $this->GoCristy_model->load_config();
    }
    ...
    public function getLatestVersion($xml_url = ''){
        if(!$xml_url){
            $xml_url = $this->config->item('GoCristy_chkverxmlurl_main');
        }
        $xml_url_bak = $this->config->item('GoCristy_chkverxmlurl_backup');
        if($this->GoCristy_model->is_url_exist($xml_url) !== FALSE){
            $xml_file = $this->GoCristy_model->get_contents_url($xml_url);
            $xml = simplexml_load_string($xml_file);
            unset($xml_file, $xml_url);
        }else if($this->GoCristy_model->is_url_exist($xml_url) === FALSE && $this->GoCristy_model->is_url_exist($xml_url_bak) !== FALSE){
            $xml_file = $this->GoCristy_model->get_contents_url($xml_url_bak);
            $xml = simplexml_load_string($xml_file);
        ...
        if($xml !== FALSE){
            ...
            $xml_cur = $this->GoCristy_model->getVersion();
    ...
    public function chkVerUpdate($cur_ver, $xml_url = ''){
        ...
        if($last_ver){
            return $this->GoCristy_model->chkVerUpdate($cur_ver, $last_ver);
        }else{
            return FALSE;
        }
    }
    ...
    public function findNextVersion($cur_txt, $xml_url = ''){
        $last_ver = $this->getLatestVersion($xml_url);
        if($last_ver){
            return $this->GoCristy_model->findNextVersion($cur_txt, $last_ver);
        }else{
            return FALSE;
        }
    }
    ...
    public function countTable($table, $search_sql = ''){
        ...
        return $this->GoCristy_model->countData($table, $search_sql);
    }
    ...
    public function getIndexData($table, $limit = 0, $offset = 0, $orderby = '', $sort = '', $search_sql = '', $groupby = '', $join_db = '', $join_where = '', $join_type = '', $sel_field = '*'){
        ...
        $count = $this->GoCristy_model->countData($table, $search_sql, $groupby, $orderby, $sort, $join_db, $join_where, $join_type);
    ...
    public function getUser($id, $type = ''){
        ...
        $rows = $this->GoCristy_model->getValue('*', 'user_admin', $sql_where, '', 1);
        if($rows !== FALSE){
            return $rows;
        }else{
            return FALSE;
    ...
    public function getUserEmail($id){
        ...
        if (!$this->cache->get('getUserEmail_'.$id)) {
            $rows = $this->GoCristy_model->getValue('email', 'user_admin', "user_admin_id", $id, 1);
            if($rows !== FALSE){
                $return = $rows->email;
            }else{
                $return = FALSE;
    ...
    public function updateUser($id){
        $query = $this->GoCristy_model->chkPassword($this->session->userdata('admin_email'), $this->input->post('cur_password', TRUE), "user_type != 'member'");
        if($query['num_rows'] !== 0){
            ...
            if($this->input->post('password') != ''){
                $this->db->set('password', $this->GoCristy_model->pwdEncypt($this->input->post('password', TRUE)), TRUE);
            ...
            if($id != 1 && $this->session->userdata('user_admin_id') != $id){
                ...
                $count = $this->GoCristy_model->countData('user_to_group', $data);
            ...
            $this->GoCristy_model->clear_file_cache('getUserEmail*', TRUE);
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    public function removeDataAndFile($table, $id_field, $id_val, $file_path, $file_sql_field){
        if($table && $id_field && $file_path && $file_sql_field){
            $file = $this->GoCristy_model->getValue($file_sql_field, $table, $id_field, $id_val, 1);
    ...
    public function sessionLoginChk(){
        ...
        if($this->session->userdata('session_id')){
            $numcount = $this->GoCristy_model->countData('user_admin', "session_id = '".$this->session->userdata('session_id')."' AND email = '".$this->session->userdata('admin_email')."'");
     ...
     *
     */
    public function gocristyCopyright(){
        /* Please do not remove or change this function */
        $GoCristy_copyright = '<br><span class="copyright">'.$this->gocristyCredit().'</span>';
        return $GoCristy_copyright;
    }
    ...
    public function coreCss($more_css = '', $more_include = TRUE){
        $core_css = '<link rel="canonical" href="' . $this->GoCristy_model->base_link(). '/' . $this->uri->uri_string() . '" />' . "\n";
    ...
    public function coreMetatags($desc_txt, $more_metatags = ''){
        ...
        $meta = array(
            ...
            array('name' => 'generator', 'content' => $this->gocristyGenerateMeta()),
    ...
    public function getSocial(){
        return $this->GoCristy_model->getSocial(FALSE);
    }
    ...
    public function updateSocial(){
        ...
        $this->GoCristy_model->clear_file_cache('getsocial*', TRUE);
    }
    ...
    public function file_upload($photo, $photo_name1, $tmp_photo, $uploaddir, $photo_id, $paramiter, $yearR = '', $maxSizeR = 1900, $resize = TRUE, $limitsize = 100){
		 ...
		 */
                if ($this->GoCristy_model->photo_xss_clean($photo) === FALSE) {
    ...
    public function getAllMenu($drop_page_menu_id = 0, $lang = '', $position = 0){
        return $this->GoCristy_model->main_menu($drop_page_menu_id, $lang, $position, TRUE);
    }
    ...
    public function findLangDataUpdate($lang_iso){
        ...
        if($query->num_rows() !== 0){
            foreach($query->result() as $rows){
                $this->db->set('lang_iso', $this->GoCristy_model->getDefualtLang(), TRUE);
                $this->db->set('timestamp_update', 'NOW()', FALSE);
    ...
    public function insertLang(){
        $arrange = $this->GoCristy_model->getLastID('lang_iso', 'arrange');
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $data = array(
    ...
    public function updateLang($id){
        $old_lang = $this->GoCristy_model->getValue('lang_iso', 'lang_iso', 'lang_iso_id', $id, 1);
        $this->load->dbforge();
    ...
    public function syncLabelLang(){
        $this->load->dbforge();
        $lang = $this->GoCristy_model->getValueArray('lang_iso', 'lang_iso', "lang_iso != ''", '');
    ...
    public function chkPageName($page_name_input){
        if($page_name_input == 'assets' || $page_name_input == 'gocristy' ||
    ...
    public function insertPage(){
        ...
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $page_url = $this->chkPageName($this->GoCristy_model->rw_link($page_name_input));
        ...
        if($this->GoCristy_auth_model->is_group_allowed('pages cssjs additional', 'backend') !== FALSE){
    ...
    public function updateForms($id){
                ...
                if($field_oldname[$i] && $form_field_id[$i]){
                    ...
                    if($field_oldname[$i] != $field_name1[$i]){
                        if($field_oldtype[$i] != 'button' && $field_oldtype[$i] != 'reset' && $field_oldtype[$i] != 'submit' && $field_oldtype[$i] != 'label' && $field_oldtype[$i] != 'file'){
                            ...
                            delete_files(FCPATH . "/photo/forms/".$this->GoCristy_model->cleanEmailFormat($form_name).'/'.$this->GoCristy_model->cleanEmailFormat($field_oldname[$i]), TRUE);
                            delete_files(FCPATH . "/photo/forms/".$this->GoCristy_model->cleanEmailFormat($form_name).'/'.$this->GoCristy_model->cleanEmailFormat($field_oldname[$i]));
                            rmdir(FCPATH . "/photo/forms/".$this->GoCristy_model->cleanEmailFormat($form_name).'/'.$this->GoCristy_model->cleanEmailFormat($field_oldname[$i]));
        ...
        $field_required = $this->input->post('field_required', TRUE);
        if(count($field_name) > 0){
            $last_arrange = $this->GoCristy_model->getLastID('form_field', 'arrange', "form_main_id = '".$id."'");
            for($i = 0; $i < count($field_name); $i++){
                if($field_name[$i]){
                    $last_arrange++;
                    $data = array(
    ...
    public function chkPluginActive($plugin_config_filename){
        if($plugin_config_filename){
            $status = $this->GoCristy_model->getValue('plugin_active', 'plugin_manager', "plugin_config_filename", $plugin_config_filename, 1);
    ...
    public function updateWidget($id){
        ...
        $this->GoCristy_model->clear_file_cache('widget_'.$this->GoCristy_model->encodeURL($this->input->post("widget_name", TRUE)));
    }
    ...
    public function updateBFSettings(){
        ...
        $this->GoCristy_model->clear_file_cache('loadBFconfig');
    }
    ...
    public function saveWhiteIP(){
        ...
        if($this->GoCristy_model->chkBFwhitelistIP($ip_address) === FALSE && !empty($ip_address)){
            $data = array(
    ...
    public function saveBlackIP(){
        $ip_address = $this->input->post('ip_address', TRUE);
        if($this->GoCristy_model->chkBFblacklistIP($ip_address) === FALSE && !empty($ip_address)){
            $data = array(
                'ip_address' => $ip_address,
                'note' => $this->input->post('note', TRUE),
            );
    ...
    public function setMaintenance() {
        ...
        $this->db->update('settings', $data);
        $this->db->cache_delete_all();
        $this->GoCristy_model->clear_file_cache('config');
    ...
    public function getSaveDraftJS() {
        ...
        $(document).ready(function () {
            $("#save_draft").click(function () {
                ...
                var urlpost = "'.$this->GoCristy_model->base_link().'/admin/admin/saveDraft'.'";
                $.ajax({
                    url: urlpost,
                    type: "POST",
                    data: fields,
    ...
    public function makePrivateKey() {
        ...
        $private_key = sha1($row->default_email . '|' . $this->GoCristy_model->base_link() . '|' . $user->password . '|gocristy|' . $gmdate);
    ...
    public function getPluginXML($xml_url = '', $key_search = '', $search = ''){       
        if(!$xml_url) $xml_url = $this->config->item('GoCristy_pluginxmlurl_main');
        if (!$this->cache->get('plugin_list_xml' . md5($xml_url))) {
            $xml_url_bak = $this->config->item('GoCristy_pluginxmlurl_backup');
            if($this->GoCristy_model->is_url_exist($xml_url) !== FALSE){
                $return = $this->GoCristy_model->get_contents_url($xml_url);
            }else if($this->GoCristy_model->is_url_exist($xml_url) === FALSE && $this->GoCristy_model->is_url_exist($xml_url_bak) !== FALSE){
                $return = $this->GoCristy_model->get_contents_url($xml_url_bak);
            }else{
                $return = FALSE;
            }
            ($this->load_config()->pagecache_time == 0) ? $cache_time = 1 : $cache_time = $this->load_config()->pagecache_time;
        ...
        if($this->cache->get('plugin_list_xml' . md5($xml_url)) !== FALSE && $key_search && $search){
            ...
            $plugin = $xml->xpath("//gocristy/plugin/" . $key_search . "[.='$search']/parent::*");
    ...
    public function chkPluginInst($plugin_config_filename){
        $count = $this->GoCristy_model->countData('plugin_manager', "plugin_config_filename = '".$plugin_config_filename."'");        
        if($count != 0){
    ...
    private function gocristyCredit(){
        /* Please do not remove or change this function */
        return $this->config->item('gocristy_credit').' Version '.$this->GoCristy_model->getVersion();
    }
    
    public function gocristyGenerateMeta(){
        /* Please do not remove or change this function */
        return 'GOCRISTY CMS | Version '.$this->GoCristy_model->getVersion();
    }
    ...
    public function pluginUninstall($plugin_config_filename){
        $plugin_file_path = $this->GoCristy_model->getPluginConfig($plugin_config_filename, 'plugin_file_path');
        $plugin_db_table = $this->GoCristy_model->getPluginConfig($plugin_config_filename, 'plugin_db_table');
        if($plugin_file_path !== FALSE && is_array($plugin_file_path) && $plugin_config_filename != 'article' && $plugin_config_filename != 'gallery'){
            foreach ($plugin_file_path as $value) {
                if($value && is_file($value) && is_writable($value)){
                    ...
                    $this->GoCristy_model->rmdir_recursive($value);
                }
            }
            ...
            $this->removeData('user_perms', "name LIKE '".$this->GoCristy_model->getPluginConfig($plugin_config_filename, 'plugin_name')."%'");
   ...
   public function dirSize($directory, $dept = 0) {
    ...
    public function insertCarouselUpload($carousel_widget_id, $carousel_type, $fileupload = '', $youtube_url = '', $photo_url = '') {
        $img_rs = $this->GoCristy_model->getValue('arrange', 'carousel_picture', "carousel_widget_id", $carousel_widget_id, 1, 'arrange', 'desc');
        if(!empty($img_rs)){
