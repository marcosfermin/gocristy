...
class GoCristy_model extends CI_Model {
    ...
    public function getVersion($version_test = '') {
        if (!$this->cache->get('getVersion'.$version_test)) {
            $con_version = $this->config->item('GoCristy_version'); /* For CMS Version */
            $con_release = $this->config->item('GoCristy_release'); /* For release or beta */
            $version = '';
            if($version_test) {
     ...
     * Please do not remove or change the fuction $this->GoCristy_admin_model->gocristyCopyright()
     *
     */
    public function gocristyCopyright($copyright_txt = '') {
        if(!$copyright_txt) {
            $copyright_txt = $this->GoCristy_admin_model->gocristyCopyright();
        }
        $row = $this->load_config();
        $html = '<span class="copyright">' . str_replace(array('%YEAR%', '%YEAR', '%Y%', '%y%'), date('Y'), $row->site_footer) . '</span>
                <small style="color:gray;">' . $copyright_txt . '</small>'; /* Please do not remove or change the fuction $this->GoCristy_admin_model->gocristyCopyright() */
        unset($row);
        return $html;
    }
    ...
    public function coreMetatags($desc_txt, $keywords, $title, $img_path = '', $more_meta = '') {
        ...
        $meta = array(
            ...
            array('name' => 'generator', 'content' => $this->GoCristy_admin_model->gocristyGenerateMeta()),
        ...
        $gplus = $this->GoCristy_model->getValue('social_url', 'footer_social', "social_name = 'google' AND active = '1'", '', 1);
        if($gplus !== FALSE && $gplus->social_url){
            $return_meta.= '<link href="' . $gplus->social_url . '" rel="publisher"/>' . "\n";
        }
        $twitter = $this->GoCristy_model->getValue('social_url', 'footer_social', "social_name = 'twitter' AND active = '1'", '', 1);
     ...
     * And set the current url into session (gocristyfrm_cururl) for use to return page. 
     *
    ...
    public function formTagToHTML($content) { /* Find the banner in content */
        ...
        if (strpos($txt_nonhtml, '[?startform_') !== false && strpos($txt_nonhtml, '[/?endform]') !== false) {
            ...
            $sess_data = array('gocristyfrm_cururl' => $cururl);
    ...
    public function addFrmToHtml($content, $frm_name, $status = '') { /* Add the form in content */
        ...
        if ($form_data && $form_data !== FALSE) {
            ...
            $fiels_file_count = $this->GoCristy_admin_model->countTable('form_field', "form_main_id = '".$form_data->form_main_id."' AND field_type = 'file'");
    ...
    public function showCaptcha() {
        ...
        if ($config->googlecapt_active && $config->googlecapt_sitekey && $config->googlecapt_secretkey) {
            if($this->uri->segment(1) == 'admin'){
                if($this->GoCristy_admin_model->getLang()){
                    $hl = '?hl='.$this->GoCristy_admin_model->getLangISOfromName($this->GoCristy_admin_model->getLang());
                }
            }else{
    ...
    public function logout($url = '') {
        ...
        $this->GoCristy_startup->clearStartup();
        if($url){
            redirect($url);
        }
        ...
        if ($query['num_rows'] !== 0) {
            ...
            if ($this->input->post('del_file')) {
                ...
                if (!empty($_FILES['file_upload']) && $_FILES['file_upload']['type'] == 'image/png' || $_FILES['file_upload']['type'] == 'image/jpg' || $_FILES['file_upload']['type'] == 'image/jpeg' || $_FILES['file_upload']['type'] == 'image/gif') {
                    ...
                    $upload_file = $this->GoCristy_admin_model->file_upload($file_f, $file_name, $this->input->post('picture', TRUE), $uploaddir, $photo_id, $paramiter);
                }
            }
    ...
    public function sendEmail($to_email, $subject, $message, $from_email, $from_name = '', $bcc = '', $reply_to = '', $alt_message = '', $attach_file = array(), $save_log = TRUE) {
        ...
        $config = array();
        $config['useragent'] = $this->GoCristy_admin_model->gocristyGenerateMeta();
    ...
    public function findFrmTag($content, $html = FALSE) {
        @ini_set("pcre.recursion_limit", "16777");
        if($html === TRUE){
            if (strpos($content, '<form ') !== false && strpos($content, ' name="csrf_gocristy"') !== false) {
                unset($content,$html);
                return TRUE;
            }else{
                unset($content,$html);
