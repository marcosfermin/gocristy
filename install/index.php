...
if (!empty($_POST) && $_POST['submitbtn'] && $_POST['baseurl'] && $_POST['dbhost'] && $_POST['dbuser'] && $_POST['dbpass'] && $_POST['dbname']) {
    ...
    $mysqli = $db->connectDB();
    $filename = 'gocristy_app.sql';
    $db->mysqli_multi_query_file($mysqli, $filename);
    $gocristymodel = new GoCristyModel;
    $md5_hash = md5(time() + mt_rand(1, 99999999));
    if ($email && $_POST['password']) {
        /* Database Insert */
        $insert_user = "INSERT INTO `user_admin` (`user_admin_id`, `name`, `email`, `password`, `user_type`, `active`, `md5_hash`, `md5_lasttime`, `pm_sendmail`, `pass_change`, `timestamp_create`, `timestamp_update`) VALUES (1, 'Admin User', '" . $email . "', '" . $gocristymodel->pwdEncypt($_POST['password']) . "', 'admin', 1, '" . $md5_hash . "', NOW(), 1, 1, NOW(), NOW())";
        $mysqli->query($insert_user);
        $update_sql = "UPDATE `settings` SET `default_email` = '" . $email . "' WHERE `settings_id` = 1";
        $mysqli->query($update_sql);
    } else {
        /* Database Insert */
        $insert_user = "INSERT INTO `user_admin` (`user_admin_id`, `name`, `email`, `password`, `user_type`, `active`, `md5_hash`, `md5_lasttime`, `pm_sendmail`, `pass_change`, `timestamp_create`, `timestamp_update`) VALUES (1, 'Admin User', 'demo@gocristy.com', '" . $gocristymodel->pwdEncypt('123456') . "', 'admin', 1, '" . $md5_hash . "', NOW(), 1, 1, NOW(), NOW())";
        $mysqli->query($insert_user);
        $update_sql = "UPDATE `settings` SET `default_email` = 'demo@gocristy.com' WHERE `settings_id` = 1";
        $mysqli->query($update_sql);
    }
    ...
    if (!$numrow) {
        ...
        $success = 1;
        $db->closeDB();
        if (isset($_COOKIE['csrf_cookie_gocristy']) || isset($_SESSION['csrf_cookie_gocristy'])) {
            unset($_COOKIE['csrf_cookie_gocristy']);
            unset($_SESSION['csrf_cookie_gocristy']);
            setcookie('csrf_cookie_gocristy', null, -1, '/');
        }
    }
}
?>
...
<html lang="en">
    <head>
        ...
        <meta name="description" content="Backend System for GOCRISTY Content Management" />
        <meta name="keywords" content="CMS, Content Management System, HTML, CSS, JS, JavaScript, framework, bootstrap, web development, thai, english" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name="author" content="gocristy" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge" />
        <meta http-equiv="Content-type" content="text/html; charset=utf-8" />
        <link href="<?php echo $curdomain; ?>/templates/admin/favicon.ico" rel="shortcut icon" type="image/ico" />
        <title>Installation System | GOCRISTY CMS</title>
        ...
        <div class="container">
            ...
            <?php } else { ?>
                <div class="row">
                    <div class="col-md-12">
                        <br><br>
                        <div class="text-center"><a href="http://www.gocristy.com" target="_blank" title="GOCRISTY CMS Official Website"><img alt="Site Logo" class="site-logo" src="<?php echo $curdomain; ?>/install/assets/images/logo.png"></a></div>
                        <br><br>
                        <div class="text-center">
                            <div class="well"><h3 class="form-signin-heading">GOCRISTY CMS Installation!</h3></div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <b>
                            ...
                            if (is_writable('../gocristy/cache/index.html')) {
                                echo '<span class="success">PASS</span>';
                                $cache_per = 1;
                            } else {
                                echo '<span class="error">FAIL</span>';
                            ...
                            if (is_writable('../gocristy/db_cache/index.html')) {
                                echo '<span class="success">PASS</span>';
                                $dbcache_per = 1;
                            } else {
                                echo '<span class="error">FAIL</span>';
                            ...
                            <div class="col-md-6">
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <input id="dbname" name="dbname" type="text" class="form-control" placeholder="DB Name for GOCRISTY-CMS" required>
                                        <br><span class="text-info">
                                            ...
                                            <a href="http://www.gocristy.com/about/contact-us" target="_blank" class="btn btn-info btn-sm" title="Contact us now!"><b>CONTACT NOW</b></a>
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <input id="timezone" name="timezone" type="text" class="form-control" placeholder="America/New_York" required>
                                ...
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <span class="text-info"><em>Please <b>blank</b> email or password field or both. If you want default backend login account<br>Email: <b>demo@gocristy.com</b> | Password: <b>123456</b></em></span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <footer>
                ...
                <div class="row">
                    <div class="col-md-8 div-copyright">
                        <?php $version->setTimezone('America/New_York'); ?>
                        <span class="copyright">Copyright &copy; <?php echo date('Y'); ?> GOCRISTY CMS Installer</span>
                        <small style="color:gray;"><br><span class="copyright">Installer for <a href="http://www.gocristy.com" target="_blank" title="GOCRISTY CMS Official Website">GOCRISTY CMS</a> Version <?php echo $version->getVersion(); ?></span></small>
                    </div>
                </div>
            </footer>
        </div>
